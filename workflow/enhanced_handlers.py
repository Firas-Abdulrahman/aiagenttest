    ...
    def _handle_ai_result(self, phone_number: str, ai_result: Dict, session: Dict, user_context: Dict) -> Dict:
        ...
        if action == 'language_selection':
            ...
            if current_step == 'waiting_for_new_quantity':
                ...
                # Force this to be item selection for removing item
                logger.info(f"🔄 Forcing quantity_selection to item_selection for remove item flow")
                return self._handle_remove_item_selection(phone_number, str(extracted_data.get('quantity', '')), session, user_context)
