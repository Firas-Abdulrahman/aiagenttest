# app.py - UPDATED with Thread Safety
import os
import json
import logging
import time
...
class ThreadSafeWhatsAppWorkflow:
    ...
    def __init__(self, config: Dict[str, str]):
        ...
        logger.info("✅ Thread-safe WhatsApp workflow initialized")
    ...
    def handle_whatsapp_message(self, message_data: Dict[str, Any]) -> Dict[str, Any]:
        """Handle WhatsApp message with thread safety"""
        try:
...
def create_flask_app():
    """Create Flask app with thread safety"""
    app = Flask(__name__)
    ...
    try:
        ...
        logger.info("✅ Thread-safe workflow initialized")
    except Exception as e:
    ...
    def home():
        ...
        <html>
        <head>
            <title>Hef Cafe WhatsApp Bot - Thread Safe</title>
            <style>
        ...
        <body>
            <div class="container">
                <h1>☕ Hef Cafe WhatsApp Bot - Thread Safe Edition</h1>
                ...
                <div class="status info">
                    ...
                    <strong>Before:</strong> Single shared handler for all users<br>
                    <strong>After:</strong> Per-user processing with isolation guarantees
                </div>

                <div class="status info" style="margin-top: 30px;">
                    <strong>System Version:</strong> 3.0.0 (Thread-Safe Edition)<br>
                    ...
                    <strong>Database Race Conditions:</strong> ❌ Prevented
                </div>

                <p style="text-align: center; color: #8B4513; font-weight: bold; margin-top: 30px;">
                    🎉 Now Supporting Multiple Users Simultaneously! 🎉
                </p>
            </div>
        </body>
        </html>
    ...
    def handle_webhook():
        """Handle webhook with thread safety"""
        try:
...
if __name__ == '__main__':
    print("🚀 Starting Thread-Safe Hef Cafe WhatsApp Bot...")
    print("🛡️ Features: User Isolation, Concurrent Processing, Race Condition Prevention")

    flask_app = create_flask_app()

    if flask_app:
        print("✅ Thread-safe bot initialized successfully!")
        print("👥 Multiple users can now order simultaneously without conflicts")

        port = int(os.environ.get('PORT', 5000))

        print(f"\n🌐 Server starting on port {port}")
        print(f"🔗 Health Check: http://localhost:{port}/health")
        print(f"🔗 Session Stats: http://localhost:{port}/session-stats")
        ...
        print("❌ Failed to initialize thread-safe bot")